...
extern int HitBox(PLAYER*, ENEMY*);	//当たり判定（四角）
...
void Player_Update() {
	...
	//画面移動制御
	//if (mPlayer.x + (70 / 2) > 640)
	//	mPlayer.x = 640 - (70 / 2);
	//if (mPlayer.x < (49 / 2))
	//	mPlayer.x = (49 / 2);

	if (mPlayer.x + (70 / 2) > 800)
		mPlayer.x = 800 - (70 / 2);
	if (mPlayer.x < (49 / 2))
		mPlayer.x = (49 / 2);
	...
	if (CheckHitKey(KEY_INPUT_SPACE) == 1 && mPlayer.jflag == false) {
		...
		if (mPlayer.jflag == true) {	//フラグがtrueなら
	...
	DrawGraph(mPlayer.x - (70 / 2), mPlayer.y - (95 / 2), gh[mPlayer.result], TRUE);
